...
static void processBlock(const void* data);
...
#define StateSize (1600 / (8*8))
#define MaxBlockSize (200 - 2 * (224 / 8))
#define BITS_KECCAK 256

/// hash
static uint64_t m_hash[StateSize];
/// size of processed data in bytes
static uint64_t m_numBytes;
/// block size (less or equal to MaxBlockSize)
static uint m_blockSize = 200 - 2 * (BITS_KECCAK / 8);
/// valid bytes in m_buffer
static uint m_bufferSize;
/// bytes not processed yet
static uint8_t m_buffer[MaxBlockSize];
/// variant
...
void keccak256_get_hash(char result[HASH_HEX_SIZE])
{
	...
	unsigned int hashLength = BITS_KECCAK / 64;
	...
	for (unsigned int i = 0; i < hashLength; i++)
		for (unsigned int j = 0; j < 8; j++) // 64 bits => 8 bytes
		{
			...
			result[result_counter++] = dec2hex[oneByte >> 4];
			result[result_counter++] = dec2hex[oneByte & 15];
		}
	// Keccak224's last entry in m_hash provides only 32 bits instead of 64 bits
	unsigned int remainder = BITS_KECCAK - hashLength * 64;
...
void processBuffer()
{
	unsigned int blockSize = 200 - 2 * (BITS_KECCAK / 8);
	// add padding
	uint offset = m_bufferSize;
	// add a "1" byte
	m_buffer[offset++] = 1;
	// fill with zeros
